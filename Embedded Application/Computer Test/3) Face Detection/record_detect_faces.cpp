...
int main() {
    ...
    while (true) {
        ...
        if (motionTriggered && noMotionCounter >= postMotionFrames) {
            ...
            if (ifstream(mp4Name)) {
                // If the main video file exists, use FFmpeg to concatenate
                string mergedFileName = "merged_output.mp4"; // New file to store merged content
                string command = "ffmpeg -i " + mp4Name + " -i " + tempMp4Name + 
                                    " -filter_complex concat=n=2:v=1:a=0 -y " + mergedFileName;
                system(command.c_str());

                // Remove the old video file and rename the merged file
